...
from dataset_loader import BaseDataset, CounterfactDataset, ZsreDataset
from context_builder import build_context, format_label, LANG_LABELS


def detect_language_by_script(text, min_distinct_langs: int = 2):
    punct = r'[。．\.！？\?!；;、,:：]'

    raw_segments = re.split(punct, text)
    segments = [seg.strip() for seg in raw_segments if seg.strip()]
    segments = segments[-1]

    lang = cld3.get_language(segments)
...
class DataEncoder:
    def __init__(self, encoder_model: str = 'BAAI/bge-m3'):
...
class DemoConstructor:
    def __init__(self, ratio: dict = None, lang: str = "en"):
        self.ratio = ratio or {"copy": 1, "update": 1}
        self.lang = lang
    ...
    def construct_demos(self,
        ...
        if paraphrase_list:
            for _ in range(self.ratio.get("update", 3)):
                demos.append(self.format_demo(new_fact, paraphrase_list[0], target_answer))
        return demos


class InContextEditor:
    def __init__(self, lm_model_name: str, device: str, use_open_model: bool):
        self.device = device
        self.use_open_model = use_open_model
        self.lm_model_name = lm_model_name
        if use_open_model == False:
    ...
    def generate(self, context: str, max_new_tokens: int = 50) -> str:
        if self.use_open_model == False:
            return self.generate_api(context, max_new_tokens=max_new_tokens)
    ...
    def generate_api(self, context: str, max_new_tokens: int = 50) -> str:
        client = OpenAI(api_key = "YOUR_API_KEY")
        response = client.chat.completions.create(
            model=self.lm_model_name,
...
class KnowledgeEditor:
    def __init__(self,
        ...
        if self.use_remake:
            # remake@counterfact
            model_dir = "/models/xlmr_rel_cls_old/best_model"
        ...
        if src_dataset is None or tgt_dataset is None:
            raise ValueError("Must provide `src_dataset` and `tgt_dataset`")
        ...
        self.editor = InContextEditor(lm_model_name, device=self.device, use_open_model=use_open_model)
        ...
        if self.use_cot:
            # clicker@counterfact
            self.encoder = SentenceTransformer('/models/bge-m3-finetuned')
        else:
    ...
    def _query_relevance(self, new_fact_src: str, test_input: str) -> str:
        ...
        return "Yes" if similarity >= self.threshold else "No"
    ...
    def _query_translate(self, text: str) -> str:
        ...
        if lang == self.tgt_lang:
            ...
            if self.tgt_lang == 'zh':
            ...
            elif self.tgt_lang == 'ja':
                prompt = (
                    "あなたはプロの翻訳者です。以下のテキスト（複数の言語を含む可能性があります）を"
                    "日本語に正確に翻訳してください。解説や追加情報は一切加えず、日本語訳のみを出力してください。\n"
                    f"テキスト：{text}\n"
                    "翻訳："
                )
                ...
                prompt = (
                    ...
                    "Übersetzen Sie den folgenden Text (der mehrere Sprachen enthalten kann) vollständig ins Deutsche. "
                    "Geben Sie nur die deutsche Übersetzung aus, ohne Erklärungen oder zusätzliche Inhalte.\n"
                    f"Text: {text}\n"
                ...
                prompt = (
                    ...
                    "Traduisez le texte suivant (qui peut contenir plusieurs langues) en français. "
                    "Fournissez uniquement la traduction française, sans explication ni contenu supplémentaire.\n"
                    f"Texte : {text}\n"
        ...
        if self.use_open_model == False:
            return self.editor.generate_api(prompt, max_new_tokens=20).strip()
        else:
    ...
    def evaluate(self,
        ...
        # Cannot get probability from a closed-source model
        if self.editor.use_open_model == False:
            # print("Skipping PPL evaluation because an OpenAI model is in use.")
            return False, {"target_new_prob": 0.5, "target_true_prob": 0.5}
        
        return None, self._orig_evaluate(
            icl_examples,
            prompt_src,
            prompt_tgt,
            new_fact_src,
