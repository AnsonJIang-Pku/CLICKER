...
class BaseDataset:
    ...
    def load_data(self) -> List[Dict[str, Any]]:
        raise NotImplementedError("Please implement the `load_data` method in the subclass.")

    def prepare_demo_texts(self) -> List[str]:
        raise NotImplementedError("Please implement the `prepare_demo_texts` method in the subclass.")
...
class WikiFactDiffDataset(BaseDataset):
    def load_data(self) -> List[Dict]:
        with open(self.filepath, "r", encoding="utf-8") as f:
            raw = json.load(f)
        unified = []
