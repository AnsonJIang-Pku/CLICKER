...
from context_builder import build_context, format_label, LANG_LABELS, predict_related
...
def evaluate_all(ke: KnowledgeEditor,
    ...
    kb_texts = []
    for entry in src_test_data:
        prompt = entry["requested_rewrite"]["prompt"].format(entry["requested_rewrite"]["subject"])
        fact = prompt + " " + entry["requested_rewrite"]["target_new"]["str"]
        kb_texts.append(fact)
    
    NULL_TOKEN = "[NULL]"
    kb_texts.append(NULL_TOKEN)
    null_id = len(kb_texts) - 1

    embeddings = ke.encoder.encode(kb_texts, convert_to_numpy=True, show_progress_bar=True)
    faiss.normalize_L2(embeddings)
    d = embeddings.shape[1]
    index = faiss.IndexFlatIP(d)
    index.add(embeddings)

    def retrieve_with_threshold(query, k=1, threshold=ke.threshold):
        qv = ke.encoder.encode([query], convert_to_numpy=True)
        faiss.normalize_L2(qv)
        D, I = index.search(qv, k)
        idx, score = I[0][0], float(D[0][0])
        if idx == null_id:
            return "", "", ""
        if score < threshold:
            return "", "", ""
        retrieved_rec = [(src_test_data[idx], float(score)) for idx, score in zip(I[0], D[0])]
        subj_src = retrieved_rec[0][0]["requested_rewrite"]["subject"]
        prompt_src = retrieved_rec[0][0]["requested_rewrite"]["prompt"].format(subj_src)
        target_new_src = retrieved_rec[0][0]['requested_rewrite']['target_new']['str']
        sep_src = LANG_LABELS.get(ke.src_lang, LANG_LABELS["en"])["sep"]
        new_fact_src = f"{prompt_src}{sep_src}{target_new_src}"
        return subj_src, prompt_src, new_fact_src

    def retrieve_no_threshold(query, k=5):
        q_emb = ke.encoder.encode([query], convert_to_numpy=True)
    ...
    def retrieve_remake(query, k=1):
        src_texts = kb_texts
        tgt_texts = [query] * len(src_texts)
        enc = ke.remake_tokenizer(
            src_texts,
            tgt_texts,
            padding=True,
            truncation=True,
        ...
        input_ids = enc["input_ids"].to(ke.device)
        attention_mask = enc["attention_mask"].to(ke.device)

        with torch.no_grad():
            logits = ke.remake_model(input_ids, attention_mask)
            probs = torch.softmax(logits, dim=-1)[:, 1].cpu().numpy()

        valid_idxs = np.where(probs >= ke.threshold)[0]
        valid_idxs = valid_idxs[valid_idxs != null_id]
        if valid_idxs.size == 0:
            return "", "", ""

        best_idx = int(valid_idxs[np.argmax(probs[valid_idxs])])
        rec = src_test_data[best_idx]["requested_rewrite"]
        subj_src      = rec["subject"]
        prompt_fmt    = rec["prompt"].format(subj_src)
        target_new    = rec["target_new"]["str"]
        sep_src       = LANG_LABELS.get(ke.src_lang, LANG_LABELS["en"])["sep"]
        new_fact_src  = f"{prompt_fmt}{sep_src}{target_new}"
        return subj_src, prompt_fmt, new_fact_src

    cache_prefix = ke.embed_cache_prefix
    indices = np.load(f"{cache_prefix}_test_indices.npy")   # shape (N, k)
    distances = np.load(f"{cache_prefix}_test_distances.npy") # shape (N, k)
    ...
    for idx, (src_rec, tgt_rec) in tqdm(enumerate(zip(src_test_data, tgt_test_data)),
        ...
        if ke.use_cot:
            ...
            subj_src, prompt_src, new_fact_src = retrieve_with_threshold(prompt_tgt, k=1)
            toc = time.perf_counter()
            print(f"Retrieval time: {toc - tic}")
        elif ke.use_remake:
            tic = time.perf_counter()
            subj_src, prompt_src, new_fact_src = retrieve_remake(prompt_tgt, k=1)
            toc = time.perf_counter()
            print(f"Retrieval time: {toc - tic}")
        else:
            subj_src, prompt_src, new_fact_src = retrieve_no_threshold(prompt_tgt, k=1)

        retrieved_demos = []
        for j, tidx in enumerate(indices[idx]):
            score = float(distances[idx, j])
            text = ke.train_texts[tidx]
            retrieved_demos.append((tidx, text, score))

        demos = []
        for rec_idx, _, _ in retrieved_demos:
            ...
            if ke.use_remake:
                tgt_constructor = DemoConstructor(ratio=ke.demo_constructor.ratio,
                                         lang=ke.tgt_lang)
                demos += ke.demo_constructor.construct_demos(nf_s, p_s, a_s, [], [])
                demos += tgt_constructor.construct_demos(nf_t, p_t, a_t, [], [])
                demos.append('\n') 
            elif ke.use_cot:
                tgt_constructor = DemoConstructor(ratio=ke.demo_constructor.ratio,
                                         lang=ke.tgt_lang)
                demos.append('\n') 
                demos += tgt_constructor.construct_demos(nf_t, p_t, a_t, rec_t.get("paraphrase_prompts", []), rec_t.get("neighborhood_prompts", []))
                demos += ke.demo_constructor.construct_demos(nf_s, p_s, a_s, rec_s.get("paraphrase_prompts", []), rec_s.get("neighborhood_prompts", []))
                demos.append('\n')
                demos += tgt_constructor.construct_demos(nf_t, p_t, a_t, [], [])
                demos += ke.demo_constructor.construct_demos(nf_s, p_s, a_s, [], [])
            else:
                demos += ke.demo_constructor.construct_demos(
                    ...
                    p_s,
                    a_s,
                    rec_s.get("paraphrase_prompts", []),
                    rec_s.get("neighborhood_prompts", [])
                )
                demos[-1] += '\n'
        ...
        for pp_src, pp_tgt in zip(src_rec.get("paraphrase_prompts", []),
            ...
            if ke.use_cot:
                subj_src, prompt_src, new_fact_src = retrieve_with_threshold(pp_tgt, k=1)
            elif ke.use_remake:
                subj_src, prompt_src, new_fact_src = retrieve_remake(pp_tgt, k=1)
            else:
                subj_src, prompt_src, new_fact_src = retrieve_no_threshold(pp_tgt, k=1)
        ...
        for np_src, np_tgt in zip(src_rec.get("neighborhood_prompts", []),
            ...
            if ke.use_cot:
                subj_src, prompt_src, new_fact_src = retrieve_with_threshold(np_tgt, k=1)
            elif ke.use_remake:
                subj_src, prompt_src, new_fact_src = retrieve_remake(np_tgt, k=1)
            else:
                subj_src, prompt_src, new_fact_src = retrieve_no_threshold(np_tgt, k=1)

            _, ns_res = ke.evaluate(
                icl_examples,
                np_src,
                np_tgt,
                new_fact_src,
            ...
            if new_fact_src or ke.use_remake:
    ...
    print(
        f"CR: {ns_consistency:.4f}, "
        f"CS: {nm_conf_change:.6f}"
    )
