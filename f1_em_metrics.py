...
def extract_after_first_punctuation(text):
    match = re.search(r'[，。！？、；：“”‘’\.,!?;:]', text)
    if match:
...
def extract_target_simple(gen_text: str, target_new: str) -> str:
    ...
    target_new, gen = target_new.lower(), gen.lower()
...
def locality(ke,
    ...
    for np_tgt in neighborhood_prompts_tgt:
        ...
        if use_cot:
            ...
            gen_loc = extract_after_first_punctuation(trans_loc)
